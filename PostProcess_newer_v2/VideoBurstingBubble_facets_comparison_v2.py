...
rmin, rmax, zmin, zmax = [-Ldomain/2, Ldomain/2, -Ldomain/2, Ldomain/2]
lw = 2

folder = 'Video_comp'  # output folder

if not os.path.isdir(folder):
    os.makedirs(folder)

for ti in range(35,nGFS):
    t = 0.01*ti
    place = "intermediate/snapshot-%5.4f" % t
    place2 = "intermediate2/snapshot-%5.4f" % t
    ...
    if not os.path.exists(place):
        ...
        if os.path.exists(name):
            ...
            if (len(segs) == 0):
                ...
                ## Drawing Facets
                line_segments = LineCollection(segs, linewidths=2.5, colors='green', linestyle='solid')
                ax.add_collection(line_segments)
                line_segments_2 = LineCollection(segs2, linewidths=2.5, colors='gray', linestyle='solid')
