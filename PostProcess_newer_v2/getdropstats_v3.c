// Getting Droplet statistics with velocity field
...
int main(int a, char const *arguments[])
{
    ...
    foreach (serial)
        if (m[] > 0)
        {
            ...
            foreach_dimension()
                b[j].x +=  vtemp * p.x;
            velx[j] += 2 * pi * p.y * Delta * Delta * f[] * u.x[];
            vely[j] += 2 * pi * p.y * Delta * Delta * f[] * u.y[];
        }
...
// Get volumes and positions which span multiple processors
    ...
    // }
     ...
     {
         int j = droptag;
         fprintf(ferr, "%g %d %g %g %g %g %g\n", t, j, v[j], b[j].x / v[j], b[j].y / v[j], velx[j] / v[j], vely[j] / v[j]);
     }
     //    }
