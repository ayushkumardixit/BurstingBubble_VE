...
def gettingfield(filename, zmin, zmax, rmax, nr, Oh):
    exe = ["./getData_v2", filename, str(zmin), str(0), str(zmax), str(rmax), str(nr), str(Oh)]
    p = sp.Popen(exe, stdout=sp.PIPE, stderr=sp.PIPE)
    stdout, stderr = p.communicate()
    temp1 = stderr.decode("utf-8")
    temp2 = temp1.split("\n")
    ...
    if not os.path.exists(place):
        ...
        if os.path.exists(name):
            ...
            if (len(segs) == 0):
                ...
                R, Z, taus, vel, U, V, taup, nz = gettingfield(place, zmin, zmax, rmax, nr, Oh)
                ...
                ## V
                cntrl2 = ax.imshow(taus, cmap="hot_r", interpolation='Bilinear', origin='lower', extent=[0, 4, -4, 4], vmax = 1.0, vmin = -2.0)
                ...
                c2.set_label(r"$\log\left(2 Oh \left( \mathbf{\mathcal{D}:\mathcal{D}} \right) \right)$",fontsize=TickLabel, labelpad=5)
