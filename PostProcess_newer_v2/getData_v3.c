...
scalar D2c[], vel[], APolyRR[], APolyQQ[], APolyZZ[], APolyRZ[], Gpd[], trace_sigma[];
scalar * list = NULL;
...
int main(int a, char const *arguments[])
{
  ...
  list = list_add(list, trace_sigma);
  ...
  */
  ...
  foreach() {
    ...
    trace_sigma[] = fa * (tau_p.x.x[] + tau_p.y.y[] + tau_qq[]);
    if (trace_sigma[] > 0.){
    trace_sigma[] = log(trace_sigma[])/log(10);
    } else {
      trace_sigma[] = -10;
    }
