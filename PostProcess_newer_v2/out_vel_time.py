...
def gettingdropstats(filename):
    ...
    return float(temp3[0]), float(temp3[1]), float(temp3[2]), float(temp3[3]), float(temp3[4]), float(temp3[5]), float(temp3[6])
# ----------------------------------------------------------------------------------------------------------------------
...
for ti in range(6, 30):
    ...
    if not os.path.exists(place):
        print("%s File not found!" % place)
                ...
                tp, jd, vd, xd, yd, velx, vely = gettingdropstats(place) # time, number od drops, volume, x, y
                print(tp)
                if not vd == 0:
                    print("Time: %4.3f, Drops: %d, Volume: %4.3f, x: %4.3f, y: %4.3f, velx: %4.3f, vely: %4.3f" % (tp, jd, vd, xd, yd, velx, vely))
                # append in output file
                    f = open("../out_vel_time.txt", "a")
                    f.write("%4.6f"  % (tp))
                    f.write(",")
                    f.write("%4.6f"  % (velx))
                    f.write("\t")
