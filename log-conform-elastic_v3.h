...
event defaults(i = 0)
{
    ...
    for (scalar s in{conform_p})
    {
        ...
        foreach_dimension() if (s.boundary[left] != periodic_bc)
        {
            s[left] = neumann(0);
            s[right] = neumann(0);
        }
...
event tracer_advection(i++)
{
    ...
    foreach ()
    {
        ...
        bool ve = (Gp[] != 0 ? true : false);
        pseudo_t A;
        if (ve)
        {
            A.x.y = conform_p.x.y[];
            foreach_dimension()
                A.x.x = conform_p.x.x[]; //(fa*conform_p.x.x[] + 1.);
        } else {
            A.x.y = 0.;
            A.x.x = 1.;
        }
        ...
        if (ve)
        {
            double Aqq = conform_qq[]; //(1. + fa*conform_qq[]);
            Psiqq[] = log(Aqq);
        } else {
            Psiqq[] = 0.;
        }
...
{
#if ViscoElastic   
  // first, we must calculate \tau from the conformation tensor using the constitution model: \tau = G_p*(A-I)

  tensor tau_p = conform_p;
#if AXI
  scalar tau_qq = conform_qq;
#endif

    foreach(){
      double fa = Gp[];
      tau_p.x.y[] = fa*(conform_p.x.y[]);
#if AXI
      tau_qq[] = fa*(conform_qq[]-1.);
#endif
      foreach_dimension()
        tau_p.x.x[] = fa*(conform_p.x.x[]-1.);
    }


  face vector av = a;
  foreach_face() if (fm.x[] > 1e-20)
  {
    double shear = (tau_p.x.y[0, 1] * cm[0, 1] + tau_p.x.y[-1, 1] * cm[-1, 1] -
                    tau_p.x.y[0, -1] * cm[0, -1] - tau_p.x.y[-1, -1] * cm[-1, -1]) /
                   4.;
    av.x[] += (shear + cm[] * tau_p.x.x[] - cm[-1] * tau_p.x.x[-1]) *
              alpha.x[] / (sq(fm.x[]) * Delta);
  }
#if AXI
  foreach_face(y) if (y > 0.)
